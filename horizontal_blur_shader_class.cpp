...
HorizontalBlurShaderClass::HorizontalBlurShaderClass()
{
    ...
    screen_size_buffer_ = nullptr;
}

HorizontalBlurShaderClass::HorizontalBlurShaderClass(const HorizontalBlurShaderClass&)
{
...
{
    bool result;
    result = Initialize_shader(device, hwnd, L"../Engine/shaders/vertex/horizontal_blur.vs", L"../Engine/shaders/pixel/horizontal_blur.ps");
    if (!result)
        return false;
    return true;
}
...
bool HorizontalBlurShaderClass::Render(ID3D11DeviceContext* device_context, int index_count, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX projection, ID3D11ShaderResourceView* texture, float screen_width)
{
    bool result;
    result = Set_shader_parameters(device_context, world, view, projection, texture, screen_width);
    if (!result)
        return false;
...
bool HorizontalBlurShaderClass::Initialize_shader(ID3D11Device* device, HWND hwnd, WCHAR* vs_filename, WCHAR* ps_filename)
{
    HRESULT result;
    ...
    D3D11_BUFFER_DESC screen_size_buffer_desc;
    ...
    result = D3DX11CompileFromFile(vs_filename, nullptr, nullptr, "Horizontal_blur_vertex_shader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, nullptr, &vertex_shader_buffer, &error_message, nullptr);
    if (FAILED(result))
    {
        if (error_message)
    ...
    result = D3DX11CompileFromFile(ps_filename, nullptr, nullptr, "Horizontal_blur_pixel_shader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, nullptr, &pixel_shader_buffer, &error_message, nullptr);
    if (FAILED(result))
    {
        if (error_message)
    ...
    screen_size_buffer_desc.Usage = D3D11_USAGE_DYNAMIC;
    screen_size_buffer_desc.ByteWidth = sizeof(SCREEN_SIZE_BUFFER_TYPE);
    screen_size_buffer_desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
    screen_size_buffer_desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    screen_size_buffer_desc.MiscFlags = 0;
    screen_size_buffer_desc.StructureByteStride = 0;

    result = device->CreateBuffer(&screen_size_buffer_desc, nullptr, &screen_size_buffer_);
    if (FAILED(result))
        return false;
...
void HorizontalBlurShaderClass::Shutdown_shader()
{
...
void HorizontalBlurShaderClass::Output_shader_error_message(ID3D10Blob* error_message, HWND hwnd, WCHAR* shader_filename)
{
    char* compile_errors;
...
bool HorizontalBlurShaderClass::Set_shader_parameters(ID3D11DeviceContext* device_context, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX projection, ID3D11ShaderResourceView* texture, float screen_width)
{
    HRESULT result;
    D3D11_MAPPED_SUBRESOURCE mapped_resource;
    MATRIX_BUFFER_TYPE* data_ptr;
    SCREEN_SIZE_BUFFER_TYPE* data_ptr2;
    ...
    result = device_context->Map(screen_size_buffer_, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_resource);
    if (FAILED(result))
        return false;

    data_ptr2 = (SCREEN_SIZE_BUFFER_TYPE*)mapped_resource.pData;
    
    data_ptr2->screen_width = screen_width;
...
void HorizontalBlurShaderClass::Render_shader(ID3D11DeviceContext* device_context, int index_count)
{
