...
{
    bool result;
    result = Initialize_shader(device, hwnd, L"../Engine/shaders/vertex/particle.vs", L"../Engine/shaders/pixel/particle.ps");
    if (!result)
        return false;
    return true;
}
...
bool ParticleShaderClass::Render(ID3D11DeviceContext* device_context, int index_count, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX projection, ID3D11ShaderResourceView* texture)
{
    bool result;
...
bool ParticleShaderClass::Initialize_shader(ID3D11Device* device, HWND hwnd, WCHAR* vs_filename, WCHAR* ps_filename)
{
    HRESULT result;
    ...
    result = D3DX11CompileFromFile(vs_filename, nullptr, nullptr, "Particle_vertex_shader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, nullptr, &vertex_shader_buffer, &error_message, nullptr);
    if (FAILED(result))
    {
        if (error_message)
    ...
    result = D3DX11CompileFromFile(ps_filename, nullptr, nullptr, "Particle_pixel_shader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, nullptr, &pixel_shader_buffer, &error_message, nullptr);
    if (FAILED(result))
    {
        if (error_message)
...
void ParticleShaderClass::Shutdown_shader()
{
    if (sample_state_)
    {
...
void ParticleShaderClass::Output_shader_error_message(ID3D10Blob* error_message, HWND hwnd, WCHAR* shader_filename)
{
    char* compile_errors;
...
bool ParticleShaderClass::Set_shader_parameters(ID3D11DeviceContext* device_context, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX projection, ID3D11ShaderResourceView* texture)
{
    HRESULT result;
...
void ParticleShaderClass::Render_shader(ID3D11DeviceContext* device_context, int index_count)
{
