...
VerticalBlurShaderClass::VerticalBlurShaderClass(const VerticalBlurShaderClass&)
{
...
{
    bool result;
    result = Initialize_shader(device, hwnd, L"../Engine/shaders/vertex/vertical_blur.vs", L"../Engine/shaders/pixel/vertical_blur.ps");
    if (!result)
        return false;
    return true;
}
...
bool VerticalBlurShaderClass::Render(ID3D11DeviceContext* device_context, int index_count, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX projection, ID3D11ShaderResourceView* texture, float screen_height)
{
    bool result;
    result = Set_shader_parameters(device_context, world, view, projection, texture, screen_height);
    if (!result)
        return false;
...
bool VerticalBlurShaderClass::Initialize_shader(ID3D11Device* device, HWND hwnd, WCHAR* vs_filename, WCHAR* ps_filename)
{
    HRESULT result;
    ...
    result = D3DX11CompileFromFile(vs_filename, nullptr, nullptr, "Vertical_blur_vertex_shader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, nullptr, &vertex_shader_buffer, &error_message, nullptr);
    if (FAILED(result))
    {
        if (error_message)
    ...
    result = D3DX11CompileFromFile(ps_filename, nullptr, nullptr, "Vertical_blur_pixel_shader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, nullptr, &pixel_shader_buffer, &error_message, nullptr);
    if (FAILED(result))
    {
        if (error_message)
...
void VerticalBlurShaderClass::Shutdown_shader()
{
...
void VerticalBlurShaderClass::Output_shader_error_message(ID3D10Blob* error_message, HWND hwnd, WCHAR* shader_filename)
{
    char* compile_errors;
...
bool VerticalBlurShaderClass::Set_shader_parameters(ID3D11DeviceContext* device_context, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX projection, ID3D11ShaderResourceView* texture, float screen_width)
{
    HRESULT result;
    ...
    data_ptr2->screen_height = screen_width;
...
void VerticalBlurShaderClass::Render_shader(ID3D11DeviceContext* device_context, int index_count)
{
