...
bool RefractionShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
 ...
 result = Initialize_shader(device, hwnd, L"../Engine/shaders/vertex/refraction.vs", L"../Engine/shaders/pixel/refraction.ps");
 if (!result)
  return false;
 return true;
}
...
bool RefractionShaderClass::Render(ID3D11DeviceContext* device_context, int index_count, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX projection, ID3D11ShaderResourceView* texture, D3DXVECTOR3 light_direction, D3DXVECTOR4 ambient_color, D3DXVECTOR4 diffuse_color, D3DXVECTOR4 clip_plane)
{
 bool result;

 result = Set_shader_parameters(device_context, world, view, projection, texture, light_direction, ambient_color, diffuse_color, clip_plane);
 if (!result)
  return false;
...
bool RefractionShaderClass::Initialize_shader(ID3D11Device* device, HWND hwnd, WCHAR* vs_filename, WCHAR* ps_filename)
{
 HRESULT result;
 ...
 result = D3DX11CompileFromFile(vs_filename, nullptr, nullptr, "Refraction_vertex_shader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, nullptr, &vertex_shader_buffer, &error_message, nullptr);
 if (FAILED(result))
 {
  if (error_message)
 ...
 result = D3DX11CompileFromFile(ps_filename, nullptr, nullptr, "Refraction_pixel_shader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, nullptr, &pixel_shader_buffer, &error_message, nullptr);
 if (FAILED(result))
 {
  if (error_message)
...
void RefractionShaderClass::Shutdown_shader()
{
 if (light_buffer_)
 {
...
void RefractionShaderClass::Output_shader_error_message(ID3D10Blob* error_message, HWND hwnd, WCHAR* shader_filename)
{
 char* compile_error;
...
bool RefractionShaderClass::Set_shader_parameters(ID3D11DeviceContext* device_context, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX projection, ID3D11ShaderResourceView* texture, D3DXVECTOR3 light_direction, D3DXVECTOR4 ambient_color, D3DXVECTOR4 diffuse_color, D3DXVECTOR4 clip_plane)
{
 HRESULT result;
 ...
 CLIP_PLANE_BUFFER_TYPE* data_ptr3;
 ...
 if (FAILED(result))
  return false;
 data_ptr3 = (CLIP_PLANE_BUFFER_TYPE*)mapped_resource.pData;
 data_ptr3->clip_plane = clip_plane;
...
void RefractionShaderClass::Render_shader(ID3D11DeviceContext* device_context, int index_count)
{
