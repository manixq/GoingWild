...
{
    bool result;
    result = Initialize_shader(device, hwnd, L"../Engine/shaders/vertex/deferred.vs", L"../Engine/shaders/pixel/deferred.ps");
    if (!result)
        return false;
    return true;
}
...
bool DeferredShaderClass::Initialize_shader(ID3D11Device* device, HWND hwnd, WCHAR* vs_filename, WCHAR* ps_filename)
{
    HRESULT result;
    ...
    result = D3DX11CompileFromFile(vs_filename, nullptr, nullptr, "Deferred_vertex_shader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, nullptr, &vertex_shader_buffer, &error_message, nullptr);
    if (FAILED(result))
    {
        if (error_message)
    ...
    result = D3DX11CompileFromFile(ps_filename, nullptr, nullptr, "Deferred_pixel_shader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, nullptr, &pixel_shader_buffer, &error_message, nullptr);
    if (FAILED(result))
    {
        if (error_message)
...
void DeferredShaderClass::Shutdown_shader()
{
...
void DeferredShaderClass::Output_shader_error_message(ID3D10Blob* error_message, HWND hwnd, WCHAR* shader_filename)
{
    char* compile_errors;
...
void DeferredShaderClass::Render_shader(ID3D11DeviceContext* device_context, int index_count)
{
    ...
    device_context->PSSetSamplers(0, 1, &sample_state_wrap_);
